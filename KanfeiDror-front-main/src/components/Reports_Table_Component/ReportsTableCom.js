...
import { Dialog, Modal,Box, IconButton, Button, Typography, Tooltip } from "@mui/material";
import {
  Edit as EditIcon,
...
const ReportsTableCom = () => {
  ...
  const [editRowUuid, setEditRowUuid] = useState("");
  const [status, setStatus] = useState('');
  const [urgency, setUrgency] = useState('');
  const [treatment, setTreatments] = useState('');
  const [oriStatus, setOriStatus] = useState('');
  const [oriUrgency, setOriUrgency] = useState('');
  const [oriTreatment, setOriTreatments] = useState('');
  ...
  const handleSubmit = async (event) => {
    ...
    const querySnapshot = await getDocs(query(collection(db, "Reports"), where("uuid", "==", editRowUuid)));
  ...
  //read and update from the database
  const handleOpenModal = (row) => {
    setEditRowUuid(row.original.uuid);
    setOpenModal(true);
    setStatus(row.original.status);
    setTreatments(row.original.treatment);
    setUrgency(row.original.urgency);
    setOriStatus(row.original.status);
    setOriUrgency(row.original.urgency);
    setOriTreatments(row.original.treatment);
  }
      ...
      <Dialog open={openModal} onClose={() => setOpenModal(false)}>
        <form dir="rtl" onSubmit={handleSubmit} className={classes.formContainer}>
          <h1 className={classes.heading}>
            דיווח מספר {editRowUuid}
          </h1>
          <div>
            <label className={classes.label}>סטטוס</label>
            <select
              value={status}
          ...
          <div>
            <label className={classes.labell}>דחיפות</label>
            <select
              value={urgency}
          ...
          <div>
            <label className={classes.labelll}>הערות של העמותה</label>
            <textarea
              className={classes.textInput}
              value={treatment}
              onChange={(e) =>
          ...
          <div className={classes.buttonContainer}>
            <button type="submit" className={`${classes.button} ${classes.submitButton}`} onClick={() => handleSubmit}>
              שמור
            </button>
            <button className={`${classes.button} ${classes.exitButton}`} onClick={() => {
              setOpenModal(false);
              setStatus(oriStatus);
              setTreatments(oriTreatment);
              setUrgency(oriUrgency);}}>
              <CloseIcon />
            </button>
          </div>
        </form>
