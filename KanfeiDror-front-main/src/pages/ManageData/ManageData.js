...
const ManageData = () => {
  ...
  const [oriMail,setOriMail] = useState("");
  const [oriPhone,setOriPhone] = useState("");
  const [oriRole,setOriRole] = useState("");
  const [openModal, setOpenModal] = useState(false);
  // const readOnlyValue = "This is a read-only value";


  // setOriCity(row.original.city);
  //   setOriMail(row.original.mail);
  //   setOriPhone(row.original.phone);
  //   setOriRole(row.original.role);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Check if the data already exists in the Firestore collection "DB"
        ...
        const querySnapshotMail = await getDocs(
          query(
            collection(db, "DB"),
            where("mail", "==", newMail),
          )
        );   
        const querySnapshotPhone = await getDocs(
          query(
            collection(db, "DB"),
            where("phone", "==", newPhone),
          )
      ...
      if (newMail !== "" && !querySnapshotMail.empty) {
        setShowEmailForm(false);
        Swal.fire({
          html: '<div dir="rtl">' + '<h1 dir="rtl">כתובת מייל כבר קיים במערכת!</h1>' + "</div>",
          icon: "error",
        });
        setNewCity("");
      setNewMail("");
      ...
      if (newPhone !== "" && !querySnapshotPhone.empty) {
        setShowEmailForm(false);
        Swal.fire({
          html: '<div dir="rtl">' + '<h1 dir="rtl">מספר טלפון כבר קיים במערכת!</h1>' + "</div>",
          icon: "error",
        });
        setNewCity("");
      setNewMail("");
      ...
      Swal.fire({
        html: '<div dir="rtl">' + '<h1 dir="rtl">מידע חדש נוסף בהצלחה!</h1>' + "</div>",
        icon: "success",
      });
  ...
  const handleOpenModal = (row) => {
    ...
    setNewPhone(row.original.phone);
...
<Dialog open={openModal} onClose={() => setOpenModal(false)}>
        <form dir="rtl" onSubmit={handleSubmit} className={classes.formContainer}>
          <h1 className={classes.heading}>
            עריכה         
          </h1>
          <div>
          <label className={classes.label}>עיר</label>
          <TextField
          ...
          <div>
            <label className={classes.label}>כתובת מייל</label>
            <TextField
            value={newMail}
            className={classes.textInput}
            onChange={handleNewEmailChange}
          ...
          <div>
            <label className={classes.label}>תפקיד/גוף מטפל</label>
            <TextField
              className={classes.textInput}
              value={newRole}
              onChange={(e) =>
          ...
          <div className={classes.buttonContainer}>
            <button type="submit" className={`${classes.button} ${classes.submitButton}`} onClick={handleSubmit1}>
              שמור
            </button>
            <button className={`${classes.button} ${classes.exitButton}`} onClick={() => {
              setOpenModal(false);
              setNewMail("");
              setNewPhone("");
              setNewRole("");}}>
          ...
          {newMail === "" && newPhone === "" && openModal == true &&(
            <p style={{ color: "red" }}>אנא וודא כי מילאת לפחות אחד מהפרטים הבאים: כתובת דוא"ל או מספר טלפון</p>
            )}
        </form>
      </Dialog>
